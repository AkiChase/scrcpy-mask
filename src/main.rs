...
// TODO 应该是模拟器本身无法接受高频指令，模仿雷电模拟器（技能键如果已有其他技能键已按下则忽略，如果是同一技能就move，如果无就down and move）
// TODO 普攻键也用技能键比较安全
// TODO 原地down up：dx,dy有个1的范围来random确定目标坐标
// TODO 封装拖拽的move：起点和终点还是固定，move要在起点和终点时分别有个加速和减速，并且微小的随机弧线移动（起点画弧的dy和dx要最终在终点抵消掉）；如果中间距离小于阈值就直接跳过（否则加几个中间点）
// TODO 所有按键都考虑一下即可（连点和多点：每次都random；方向轮盘：拖拽的dy和dx可以略大一点）
// TODO 按键编辑时未考虑滚动条的偏移量，导致存在偏移量时长按后按钮位置跳变
// TODO 鼠标滚轮要优化，使用节流的方式吧，不要考虑滚动的阈值了
