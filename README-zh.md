...
- [x] Scrcpy 控制协议（映射）
- [x] Scrcpy 视频协议（投屏）
- [x] 保持画面比例的无边框全屏（`F11` 或 `Alt+Enter` 切换，`Escape` 退出；RawInput 模式不拦截快捷键）
- [x] 按键映射配置可视化
...
- [ ] 手柄按键映射（等待赞助支持❤️）
- [x] 外部程序控制（HTTP API + WebSocket，可管理设备、映射配置和应用设置，含窗口同步示例），见[外部控制](https://github.com/AkiChase/scrcpy-mask-external-control)
- [ ] 帮助文档
